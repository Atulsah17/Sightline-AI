"""Sightline core pipeline: open-vocab detect -> track -> ROI counting -> rules/alerts -> annotate -> report -> dataset.

Runs the same locally (CPU) and on Modal (GPU). Outputs, regions, and rules are all optional.
"""
...
def _prepare_rules(rules):
    ...
    for r in rules or []:
        out.append({
            "name": r.get("name") or "Alert",
            "metric": r.get("metric", "screen"),   # zone | line | screen
            "region": r.get("region"),
            "cls": r.get("class") or r.get("cls"),
            "threshold": float(r.get("threshold", 1)),
            "fired": False, "first_time": None, "peak": 0,
        })
    return out
...
def _eval_rules(rules, t, frame_total, frame_class, frame_zone, lines):
    ...
    for R in rules:
        ...
        else:  # screen: total objects (optionally of one class) in the frame
...
def process(
    ...
    stats = {
        "frames_processed": frame_idx, "fps": round(fps, 2),
        "duration_s": round(frame_idx / fps, 2), "classes": classes,
        "unique_objects": dict(unique_per_class), "total_detections": len(detections),
...
if __name__ == "__main__":
    ...
    out = process(
        ...
        outputs={"video": False, "report": True, "dataset": False},
        regions=[{"type": "line", "a": [0.1, 0.5], "b": [0.9, 0.5], "name": "Doorway"}],
        rules=[{"name": "Crowd alert", "metric": "screen", "threshold": 1},
               {"name": "Capacity", "metric": "line", "region": "Doorway", "threshold": 3}],
        max_frames=80,
    )
